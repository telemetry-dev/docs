...
| `session_mode` (unreleased) | `"explicit"` or `"process"` | None | `"explicit"` | Optionally groups otherwise uncorrelated SDK spans and logs for one client initialization. |
...
Root spans are independent by default. For a single-conversation CLI process, the upcoming `session_mode="process"` option generates a session ID when the SDK initializes:

```python
init(session_mode="process")
```
...
Do not use process sessions to group unrelated requests in a long-running server. Supply a session ID per conversation with `propagate_attributes()` instead.
